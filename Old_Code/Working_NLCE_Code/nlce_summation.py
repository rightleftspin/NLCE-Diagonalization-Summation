...
def wynn_resummation(input_dict, final_order, weighted_property_dict):
    """
    This function takes in the weighted property dictionary
    and returns the resummed property using the euler method
    starting at the resummation order
...
#        plt.plot(temp_grid, euler_resummation(input_dict, weight_dict_min_one)[magnetization_index, :], 'b--', label = f"{final_order - 1} sites (Euler)")
        ...
        #plt.plot(temp_grid, weight_dict[final_order], 'r', label = f"{final_order} sites")
        #plt.plot(temp_grid, weight_dict[final_order - 1], 'g', label = f"{final_order - 1} sites")
        #plt.plot(temp_grid, weight_dict[final_order - 2], 'b', label = f"{final_order - 2} sites")
        ...
        ##plt.plot(temp_grid, wynn_resummation(input_dict, weight_dict), 'c', label = f"{final_order} sites (Wynn)")

        #weight_dict_min_one = {k:weight_dict[k] for k in range(final_order) if k in weight_dict}

        #plt.plot(temp_grid, euler_resummation(input_dict, weight_dict), 'b-', label = f"{final_order} sites (Euler)")
        #plt.plot(temp_grid, euler_resummation(input_dict, weight_dict_min_one), 'b--', label = f"{final_order - 1} sites (Euler)")
        ...
        temp_index = 28
        plt.plot(mag_grid, weight_dict[final_order][:, temp_index], 'b')
        plt.plot(mag_grid, wynn_resummation(input_dict, 10, weight_dict)[:, temp_index], 'g-', label = f"{final_order}Max_Wynn6 T = {temp_grid[temp_index]:.4f}")
#        plt.plot(mag_grid, wynn_resummation(input_dict, 13, weight_dict_min_one)[:, temp_index], 'g--', label = f"{final_order - 1}Max_Wynn6 T = {temp_grid[temp_index]:.4f}")

#        temp_index = 40
#        plt.plot(mag_grid, wynn_resummation(input_dict, 14, weight_dict)[:, temp_index], 'r-', label = f"{final_order}Max_Wynn6 T = {temp_grid[temp_index]:.4f}")
#        plt.plot(mag_grid, wynn_resummation(input_dict, 13, weight_dict_min_one)[:, temp_index], 'r--', label = f"{final_order - 1}Max_Wynn6 T = {temp_grid[temp_index]:.4f}")
#
#        temp_index = 40
#        plt.plot(mag_grid, wynn_resummation(input_dict, 14, weight_dict)[:, temp_index], 'b-', label = f"{final_order}Max_Wynn6 T = {temp_grid[temp_index]:.4f}")
#        plt.plot(mag_grid, wynn_resummation(input_dict, 13, weight_dict_min_one)[:, temp_index], 'g--', label = f"{final_order - 1}Max_Wynn6 T = {temp_grid[temp_index]:.4f}")
#
        plt.xlabel("h")
        plt.ylabel("M")
        plt.title(f"{property_name} vs h for J = {tunneling_strength[0]}")
        #plt.ylim([0, 0.8])
        #plt.xlim([0, 6])
