...
leftAdjecencyList = [1, 0, clusterSize, -clusterSize, clusterSize + 1, 0]
rightAdjecencyList = [0, -1, clusterSize, -clusterSize, 0, -(clusterSize + 1)]
totalAdjecencyList = [1, -1, clusterSize, -clusterSize, clusterSize + 1, -(clusterSize + 1)]
###########################################
...
def vSimple(graph, subgraph, neighbors, guardingSet, size, graphFunc):
    ...
    newGuardingSet = deepcopy(guardingSet)
    while (len(neighbors) > 0):
        # create a subgraph by adding the neighbor
        neighbor = neighbors.pop()
        subgraph = subgraph | {neighbor}
        ...
        addNeighbors = graph[neighbor].difference(subgraph).difference(newGuardingSet)
        ...
        newNeighbors = neighbors | addNeighbors
        # Recursively call this algorithm again with the new subgraph and new neighbor set
        end = vSimple(graph, subgraph, newNeighbors, newGuardingSet, size, graphFunc)
        if end:
        ...
        newGuardingSet.add(neighbor)

        # If the guarding set ever gets too big, break out of this loop
        if (len(graph) - len(newGuardingSet)) < size:
            return(hasIntLeaf)
...
start = time.time()
enumerateGraph(setLattice, clusterSize, {startingPoint}, addCluster)
...
writeFile = open(f"NLCE_Triangles_1_{clusterSize}_test.txt", 'w')
